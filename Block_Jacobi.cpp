...
crs_matrix_type Create_R_tilde(crs_matrix_type R,
    ...
    // For each value in entries, get the number of occurrences in entries.
    Vector occurences(n*n, 0);
    for (int i = 0; i < R_entries.extent(0); i++)
    {
        occurences[R_entries(i)] += 1;
    }

    // For each value in values, divide by the number of occurrences
    for (int i = 0; i < R_values.extent(0); i++)
    {
        R_tilde_values(i) = R_values(i) / occurences[R_entries(i)];
    }

    // Create a new graph
    graph_type R_tilde_graph(R_tilde_entries, R_tilde_row_map);

    // Create a new crs_matrix_type with the new values
    crs_matrix_type R_tilde("R_tilde", n*n, R_tilde_values, R_tilde_graph);
...
struct Functor_BatchedSerialLU{
    ...
    Functor_BatchedSerialLU(bsr_matrix_type block_matrix_) 
    ...
    void operator()(const int i) const {
        ...
        KokkosBatched::SerialLU<AlgoTagType>::invoke(block);
...
struct Functor_BatchedSerialSolveLU{
    bsr_matrix_type D;
    ViewVectorType U;
    

    Functor_BatchedSerialSolveLU(bsr_matrix_type D_, ViewVectorType U_) 
    : D(D_), U(U_) {}
    ...
    void operator()(const int i) const {
        ...
        KokkosBatched::SerialSolveLU<TransType, AlgoTagType>::invoke(block_D, block_U);
...
void Jacobi(int n, 
    ...
    using AlgoTagType = KokkosBatched::Algo::LU::Unblocked;
    using TransType = KokkosBatched::Trans::NoTranspose;

    Kokkos::parallel_for("LU decomposition", range_policy(0, num_blocks*num_blocks), 
                         Functor_BatchedSerialLU<AlgoTagType>(A_hat));
    ...
    while (residual > tolerance)
    {
        ...
        // r_k = - A * u
        //KokkosSparse::spmv("N", -1.0, A, u, 0.0, r_k);
	...
	Kokkos::parallel_for(range_policy(0, n*n), KOKKOS_LAMBDA(const int i) {
       ...
       // KokkosSparse::spmv("N", 1.0, R, r_k, 0.0, r_k_extended);

	Kokkos::parallel_for(range_policy(0, num_blocks*num_blocks), KOKKOS_LAMBDA(const int i) {
        ...
        Kokkos::parallel_for("Solve LU systems", range_policy(0, num_blocks*num_blocks), 
                             Functor_BatchedSerialSolveLU<TransType, AlgoTagType>(A_hat, r_k_extended));

        // Calculate r_k = R_T_tilde * r_k_extended
        KokkosSparse::spmv("T", 1.0, R_tilde, r_k_extended, 0.0, r_k);
...
int main(int argc, char *argv[])
{
	...
	if ((block_size * num_blocks - n)%(num_blocks - 1) != 0){
        ...
        // Jacobi iteration
        Jacobi(n, h, u_start, b, A, A_hat, R, R_tilde, block_size, num_blocks, problem);
